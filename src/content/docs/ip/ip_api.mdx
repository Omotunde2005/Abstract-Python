...
- **ip_address (String):** The ip address you want to get location data for. The IP API supports both IPv4 and IPv6 addresses.
...
api = IpAPI(api_key)
print(api.ip_info(ip_address=ip_address))
```
...
api = IpAPI(api_key)
print(api.ip_info(ip_address=ip_address, fields=fields))
```
