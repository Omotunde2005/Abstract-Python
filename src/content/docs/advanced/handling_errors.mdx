...
Errors are unavoidable with dynamic user inputs. The `status` field in each response allows you to verify the
state of your request. [Read more here](/advanced/errors)
...
def validate_vat(vat_number):
    try:
        ...
        # Return an error message e.g Only string characters are required not number
    else:
